...
{
    internal class SCP323_1AI : EnemyAI, ISingletonEnemy, IVisibleThreat
    {
            ...
            switch (currentBehaviourStateIndex)
            {
                ...
                case (int)State.Hunting:
                    ...
                    if (enraged)
                    {
                        ...
                        {
                            if (enraged) { SetEnragedRpc(false); }
                        }
                        return;
                    }
                ...
                case (int)State.Eating:
                    ...
                    if (targetPlayerCorpse != null && targetPlayerCorpse.grabBodyObject != null && !targetPlayerCorpse.grabBodyObject.isHeld && !targetPlayerCorpse.isInShip)
                    {
                        if (!inSpecialAnimation && Vector3.Distance(targetPlayerCorpse.grabBodyObject.transform.position, transform.position) <= 1f)
                        {
                            ...
                            EatPlayerBodyRpc(targetPlayerCorpse.playerScript.actualClientId);
                            return;
                        }
        ...
        [Rpc(SendTo.Everyone)]
        public void SetEnragedRpc(bool value)
        {
            enraged = value;
        }

        [Rpc(SendTo.Server)]
        public void PlayerDamagedRpc(ulong clientId)
        {
            if (!IsServer) { return; }
        ...
        [Rpc(SendTo.Everyone)]
        void IncreaseFearOfNearbyPlayersRpc(float value, float distance)
        {
        ...
        [Rpc(SendTo.Everyone)]
        void DoAnimationRpc(string animationName)
        {
            creatureAnimator.SetTrigger(animationName);
        }

        [Rpc(SendTo.Everyone)]
        void DoAnimationRpc(string animationName, bool value)
        {
            creatureAnimator.SetBool(animationName, value);
        }

        [Rpc(SendTo.Everyone)]
        public void EatPlayerBodyRpc(ulong clientId)
        {
            creatureAnimator.SetTrigger("eat");

            targetPlayerCorpse = PlayerFromId(clientId)?.deadBody;
            if (targetPlayerCorpse == null) { return; }
        ...
        public static void DamagePlayerPostfix(PlayerControllerB __instance, int damageNumber)
        {
            try
            {
                ...
                if (SCP323_1AI.Instance != null)
                {
                    ...
                    {
                        SCP323_1AI.Instance.PlayerDamagedRpc(__instance.actualClientId);
                    }
                }
            }
