...
using static HeavyItemSCPs.SCP.SCP178.SCP178Behavior;
...
namespace HeavyItemSCPs.SCP.SCP178
{
    public class SCP1781AI : NetworkBehaviour // UPDATE: Turn this into a monobehavior instead and always have them spawned, similar to what we planned with the rats
    {
        ...
        // Configs
        public const float postObservationTime = 5f;
        public const float wanderingRadius = 5f;
        public const float wanderWaitTime = 5f;
        public const float observationGracePeriod = 0.5f;
        public const float distanceToAddAnger = 10f;
        public const int maxAnger = 100;
        public const int playerDamage = 40;
        public const float activeTimeAfterVisible = 15f;
        public const float renderDistance = 20f;

        public static BoundedRange minMaxCount = new BoundedRange(50, 100);
        ...
        {
            minMaxCount = PluginInstance.Config.Bind("SCP-178-1 Options", "SCP-178-1 | Min-Max Count", new BoundedRange(50, 100), "The minimum/maximum number of SCP-178-1 instances that can be spawned.").Value;
        }
        ...
        {
            if (!IsServerOrHost) { return; }
            CollideWithPlayerRpc(clientId, UnityEngine.Random.Range(0, 1));
        }

        [Rpc(SendTo.Everyone)]
        public void CollideWithPlayerRpc(ulong clientId, int attackIndex)
        {
            PlayerControllerB? player = PlayerFromId(clientId);
